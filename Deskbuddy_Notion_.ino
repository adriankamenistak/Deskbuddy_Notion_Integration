...
const char* NOTION_SECRET = "YOUR_NOTION_SECRET"; // Replace with  your notion api key or code
const char* NOTION_PAGE_ID = "YOUR_PAGE_ID"; // Replace with your notion page id
...
// Notion notes
static time_t lastNotionFetch = 0;
static const uint32_t NOTION_INTERVAL_SEC = 0.5 * 60; // poll every 0.5 min
...
bool fetchNotionNote() {
  ...
  HTTPClient https;
  String url = "https://api.notion.com/v1/blocks/" + String(NOTION_PAGE_ID) + "/children";
  if (!https.begin(client, url)) return false;

  https.addHeader("Authorization", "Bearer " + String(NOTION_SECRET));
  ...
  String result = "";
  for (JsonObject block : doc["results"].as<JsonArray>()) {
    String type = block["type"].as<String>();
    if (block.containsKey(type) && block[type].containsKey("rich_text")) {
      for (JsonObject rt : block[type]["rich_text"].as<JsonArray>()) {
        result += rt["plain_text"].as<String>();
      }
      result += "\n";
    }
  }

  if (result.length() == 0) result = "No notes yet.";
  if (result != notesText) {
    notesText = result;
    notesDirty = true;
  }

  lastNotionFetch = time(nullptr);
  return true;
}

void ensureNotionNotes() {
  time_t nowT = time(nullptr);
  if ((nowT - lastNotionFetch) > NOTION_INTERVAL_SEC && WiFi.status() == WL_CONNECTED) {
    fetchNotionNote();
  }
}
